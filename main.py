...
def muovi_aliena(aliena):
    ...
    # Змініть напрямок і опустіть корабель, коли він торкнеться країв
    if x > 290 or x < -290:
        aliena.direction *= -1
        y = aliena.ycor()
        y -= 40
...
def aggiorna_punteggio():
    score_display.clear()
    score_display.write("Рахунок: {}".format(punteggio), align="left", font=("Courier", 12, "normal"))

def aggiorna_barra_vita(barra_vita, vita):
    barra_vita.clear()
    barra_vita.write("Життя : {}".format(vita), align="right", font=("Courier", 12, "normal"))
...
# Панель життя космічного корабля прибульців
...
bullet.penup()
bullet.speed(10)
bullet.shapesize(stretch_wid=0.5, stretch_len=0.5)  # Зробіть снаряд інопланетного корабля тоншим
bullet.hideturtle()
bullet_state = "ready"
...
while True:
    ...
    # Перевірте, чи потрапляє снаряд гравця в корабель прибульців
